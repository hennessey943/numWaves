function [dx,err] = inClassWaveFixed( N,xa,xb,c,tf,iPlot )
  %N = 50;
  %xa = 0;
  %xb = 1;
  %c = 1;
...
function un = setBCs( un,ia,ib,dx,t)
  un(ia-1) = 2*un(ia)-un(ia+1)-4*dx^2*cos(t);
  un(ib+1) = 2*un(ib)-un(ib-1)-4*dx^2*cos(2)*cos(t);
  return
end
...
function [unm1,un] = setICs( x,ia,ib,dx,dt,c )
  ...
  for i = ia:ib
    ...
    %un(i) = f0+dt*g0+0.5*dt^2*(utt+h0);
    un(i)=f0-.5*dt^2*cos(2*x(i));
  end
  un = setBCs( un,ia,ib,dx,dt );
  return
end